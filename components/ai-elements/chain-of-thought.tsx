...
export const ChainOfThoughtHeader = memo(
  ({
    ...
    const buttonClassName = useMemo(
      () =>
        cn(
          "group/row flex h-[2.625rem] cursor-pointer flex-row items-center justify-between gap-4 rounded-xl px-3 py-2 text-muted-foreground transition-colors duration-200 hover:text-foreground"
        ),
      []
    );
    ...
    // Get the first 3 tools for display (simplified - just show dots)
    const displayTools = useMemo(
      () =>
        tools.slice(0, 3).map((toolName) => ({
          toolName,
        })),
      [tools]
    );
