...
const TooltipContent = memo(
  ({
    ...
    <Portal>
      <Content
        className={cn(
          "fade-in-0 zoom-in-95 data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-[110] w-fit animate-in text-balance rounded-md bg-popover px-3 py-1.5 font-medium text-popover-foreground text-xs data-[state=closed]:animate-out",
          className
        )}
      ...
      >
        {children}
        <Arrow className="z-[110] size-2.5 translate-y-[calc(-50%_-_2px)] rotate-45 rounded-[2px] bg-popover fill-popover" />
      </Content>
    </Portal>
  )
);
