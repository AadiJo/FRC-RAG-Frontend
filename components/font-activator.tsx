...
const normalizedRegistry: Record<string, () => Promise<{ fontVar: string }>> =
  Object.freeze(
    Object.fromEntries(
...
export function FontActivator({ themeState }: Props) {
  ...
  const names = useMemo(() => {
    ...
    for (const name of [lightSans, lightMono, darkSans, darkMono]) {
      if (name) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const results = (await Promise.all(promises)).filter((v): v is string =>
        Boolean(v)
      );
  ...
  useEffect(() => {
    ...
    for (const cls of current) {
      if (!previouslyApplied.has(cls)) {
        toAdd.push(cls);
      }
    }

    for (const cls of previouslyApplied) {
      if (!current.has(cls)) {
        toRemove.push(cls);
      }
    }
