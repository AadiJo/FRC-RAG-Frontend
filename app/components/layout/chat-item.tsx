...
const ChatItemComponent = function ChatItemComponent({
  ...
  const renderContent = () => {
    ...
    return (
      <Link
        className={cn(
          "group relative flex h-9 w-full items-center overflow-hidden rounded-lg px-2 py-1 text-sm outline-none",
      ...
      >
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    router.push(`/chat/${originalChatId}`);
                  }}
                  type="button"
                >
        ...
        <div
          className={cn(
            "absolute top-0 right-0 bottom-0 z-10 flex items-center justify-end text-muted-foreground transition-all duration-200",
            "group-hover:bg-accent dark:group-hover:bg-muted",
        ...
        >
          <div className="pointer-events-none absolute top-0 right-[100%] bottom-0 h-12 w-8 bg-gradient-to-l from-accent to-transparent opacity-0 transition-opacity duration-200 group-focus-within:opacity-100 group-hover:opacity-100 dark:from-muted" />
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
