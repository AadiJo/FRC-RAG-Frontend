...
export function DialogShare() {
  ...
  const canShare = useMemo(
    () => Boolean(chatId) && pathname?.startsWith("/chat/"),
    [chatId, pathname]
  );

  // Temporary toggle to disable the Share feature (mirrors file upload disabling)
  // Set to `false` to restore the original share behavior.
    ...
    return (
      <Popover>
        <Tooltip>
          <TooltipTrigger asChild>
            <PopoverTrigger asChild>
              <Button
                ...
                className="h-8 w-8 shrink-0 rounded-lg text-white/60 opacity-50 hover:bg-white/10 hover:text-white"
                size="icon"
                type="button"
                variant="ghost"
              >
          ...
          <TooltipContent>Share disabled</TooltipContent>
        </Tooltip>
        <PopoverContent className="p-2">
          <div className="text-secondary-foreground text-sm">
            Chat sharing is temporarily disabled.
            <br />
