  ...
  return (
    <div className="flex flex-col pt-2 pb-8">
      {pinnedChats.length === 0 && Object.keys(groupedChats).length === 0 && (
        <span className="px-2 text-white/50 text-xs">No chats yet.</span>
      )}
      {/* Pinned Chats Section */}
      {pinnedChats.length > 0 && (
        <div className="relative flex w-full min-w-0 flex-col">
          <h3 className="flex h-8 shrink-0 select-none items-center rounded-md px-2 font-medium text-white/50 text-xs uppercase tracking-wider outline-none">
        ...
        (groupKey) =>
          hasChatsInGroup(groupedChats, groupKey) && (
            <div
              ...
              key={groupKey}
            >
              <h3 className="flex h-8 shrink-0 select-none items-center rounded-md px-2 pt-6 pb-3 font-medium text-white/50 text-xs uppercase tracking-wider outline-none">
                {groupKey}
              </h3>
              <ul className="flex w-full min-w-0 flex-col gap-1 text-sm">
