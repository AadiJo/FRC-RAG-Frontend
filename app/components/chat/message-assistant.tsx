...
import { RAGRelatedImages } from "./rag-related-images";
import { RAGTextContent } from "./rag-text-content";
...
const isNoticePart = (part: unknown): part is NoticeUIPart =>
  typeof part === "object" &&
  part !== null &&
  "type" in part &&
...
const renderTextPart = (
  ...
  // Use RAG-aware text content component if we have RAG images.
  // This renders images only when their [img:...] placeholders appear in the streamed text.
  if (ragImageMap.length > 0) {
    return (
      <RAGTextContent
        id={id}
        index={index}
        key={`text-${index}`}
        ragImages={ragImageMap}
        text={part.text}
      />
    );
  }
...
// Connector tools have been removed - this is a placeholder for any future tool rendering
// that doesn't rely on the removed connector system
...
const renderToolPart = (
  ...
  // Connector tools have been removed - return null for unknown tool types
  return null;
};
...
const renderPartDirectly = (
  ...
  switch (part.type) {
    ...
    default:
      ...
      if (isNoticePart(part)) {
        return renderNoticePart(part, index);
      }
  ...
  switch (part.type) {
    ...
    default:
      if (part.type.startsWith("tool-")) {
        ...
        // Use tool type as label (connectors removed)
      ...
      if (isNoticePart(part)) {
        return (
          <ChainOfThoughtStep key={partKey} label="Notice" status="complete">
            {renderNoticePart(part, index)}
          </ChainOfThoughtStep>
        );
      }
      // Skip rendering boundary markers (they're only for detection)
...
const renderErrorPart = (part: ErrorUIPart, index: number) => (
  <div
    className="mt-4 flex items-center justify-center rounded-lg bg-red-500/15 px-4 py-3 text-red-900 text-sm dark:text-red-400"
    key={`error-${index}`}
    role="alert"
  >
...
const renderNoticePart = (part: NoticeUIPart, index: number) => (
  <output
    className="mt-4 flex items-center justify-center rounded-lg bg-red-500/15 px-4 py-3 text-red-900 text-sm dark:text-red-400"
    key={`notice-${index}`}
  >
...
function MessageAssistantInner({
  ...
  // If the server indicates some images were skipped, we may have partial
  // results. Still render any images we *do* have rather than hiding all.
  const effectiveRagImageMap = ragImageMap;

  // Log response statistics to the console with colored output for debugging
  useEffect(() => {
    if (!metadata) {
      return;
    }
    ...
    try {
      ...
      const modelName =
        (md.modelName as string) ||
        (md.modelId as string) ||
        displayModel ||
        "unknown";
      const serverMs =
        Number(md.serverDurationMs ?? md.serverDuration ?? 0) || 0;
      const inputTokens = Number(md.inputTokens ?? 0) || 0;
      const outputTokens = Number(md.outputTokens ?? 0) || 0;
      const reasoningTokens = Number(md.reasoningTokens ?? 0) || 0;
      const cachedInputTokens = Number(md.cachedInputTokens ?? 0) || 0;
      const totalTokens =
        Number(md.totalTokens ?? inputTokens + outputTokens) || 0;
      const tps = serverMs > 0 ? totalTokens / (serverMs / 1000) : Number.NaN;

      const titleStyle =
        "background:linear-gradient(90deg,#7c3aed,#06b6d4);color:#fff;padding:4px 8px;border-radius:6px;font-weight:700";
      const labelStyle = "color:#0f172a;font-weight:600";
      const valStyle = "color:#111827;font-weight:700";
      const _dim = "color:#6b7280";

      console.log(`%cResponse Stats %c${modelName}`, titleStyle, "");
      console.log(
        "%cinput:%c %d   %coutput:%c %d   %ctotal:%c %d   %ctps:%c %s   %cserverMs:%c %dms   %creasoning:%c %d   %ccachedIn:%c %d",
        labelStyle,
        valStyle,
        inputTokens,
        labelStyle,
        ...
        labelStyle,
        valStyle,
        Number.isFinite(tps) ? `${tps.toFixed(2)} tok/sec` : "n/a",
        labelStyle,
        valStyle,
        serverMs,
        labelStyle,
  ...
  return (
    <Message
      className={cn(
        "group flex w-full max-w-3xl flex-1 items-start gap-4 px-6 pb-6",
    ...
    >
      <div className={cn("flex w-full flex-col gap-2", isLast ? "pb-8" : "")}>
        ...
        {/* Render RAG related images section - only after streaming completes */}
        {status !== "streaming" && ragRelatedImages.length > 0 && (
          <RAGRelatedImages
            images={ragRelatedImages}
            imagesSkipped={ragImagesSkipped}
          />
        )}

        <MessageActions
          className={cn(
            "pointer-events-auto flex gap-0 transition-opacity",
            isTouch
              ? "opacity-100"
...
const areEqual = (prev: MessageAssistantProps, next: MessageAssistantProps) => {
  return (
    ...
    prev.ragImageMap === next.ragImageMap &&
    prev.ragRelatedImages === next.ragRelatedImages &&
    prev.ragImagesSkipped === next.ragImagesSkipped
